...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // --- FUNCIÓN DE NORMALIZACIÓN (IGNORA TILDES Y MAYÚSCULAS) ---
    const normalizar = (texto: string) => 
      texto ? texto.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim() : "";

    const datos = filas.filter(f => f[1] && normalizar(f[1]) === normalizar(nombreInstitucion));
    
    if (datos.length === 0) return res.status(404).json({ error: 'No se encontraron datos para esta institución. Verifique el nombre.' });
    ...
    await transporter.sendMail({
      ...
      to: destinoCorreo,
      subject: `📊 Informe Diagnóstico Final: ${nombreInstitucion}`,
      text: `Adjunto enviamos el diagnóstico cualitativo automatizado para ${nombreInstitucion}.`,
