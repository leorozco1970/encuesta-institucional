import { google } from 'googleapis';
// @ts-ignore
import nodemailer from 'nodemailer';
import { jsPDF } from 'jspdf';
import 'jspdf-autotable';

export default async function handler(req: any, res: any) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Método no permitido' });
  
  const { nombreInstitucion, destinoCorreo } = req.body;

  try {
    const auth = new google.auth.GoogleAuth({
      credentials: {
        client_email: "sheets-save@encuesta-institucional-489716.iam.gserviceaccount.com",
        private_key: "-----BEGIN PRIVATE KEY-----\\giC228Rdg+oo/Oa9UAB0xNlYLJLHFq+4KtYGOWCclo4IMwsFTmgfC7s8meO0jkOS\noA1nYYMc9yb6xmEnV+UyVaR0aQKBgQDSo5QheGoz6OvzcdOaBB7JwHRb5+FwsBBU\nueRd9qHqBK4N8Nyhm92giBdBTZyzGIYXbIi4+Dt/Ia+xZJrLhYARtCOjxDzx/FWC\Tu1fTHlyKM5/OpwcJ6clbWJfQAoXTD4zvRwoQU0+jKUSzxrAx76Y4ZWYSt9LHOWY\sw25OjnfwwKBgHVERVd9e+Tj8xLPtNiURrKg/To/e5+aUwDQn8mljYhYYOqrxS7e\ \nWqYxjQAOKgIHnTddyMqGE0Ns\n-----END PRIVATE KEY-----\n".replace(/\\n/g, '\n'),
      },
      scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'],
    });

    const sheets = google.sheets({ version: 'v4', auth });
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: '15oJuvgGQIFE4cbGR3VU_zZ6sEco4gKDlUa6j0aoJj_g',
      range: "'Hoja 1'!A:P",
    });

    const filas = response.data.values || [];
    const datos = filas.filter(f => f[1] && f[1].toUpperCase() === nombreInstitucion.toUpperCase());

    if (datos.length === 0) return res.status(404).json({ error: 'No se encontraron datos para esta institución' });

    // 1. Conteo de participación
    const porRol = {
      Directivos: datos.filter(f => f[3] === 'Directivos').length,
      Docentes: datos.filter(f => f[3] === 'Docentes').length,
      Padres: datos.filter(f => f[3] === 'Padres').length,
      Estudiantes: datos.filter(f => f[3] === 'Estudiantes').length,
    };

    // 2. GENERACIÓN DEL PDF
    const doc = new jsPDF();
    const logoAzul = [30, 58, 138];

    doc.setFillColor(logoAzul[0], logoAzul[1], logoAzul[2]);
    doc.rect(0, 0, 210, 40, 'F');
    
    doc.setTextColor(255, 255, 255);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(22);
    doc.text("INFORME DE ARMONIZACIÓN", 105, 25, { align: "center" });

    doc.setTextColor(0, 0, 0);
    doc.setFontSize(16);
    doc.text(`${nombreInstitucion.toUpperCase()}`, 20, 60);
    
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text(`Fecha de generación: ${new Date().toLocaleDateString()}`, 20, 70);

    // Tabla de participación
    (doc as any).autoTable({
      startY: 80,
      head: [['Rol Institucional', 'Total Encuestados']],
      body: Object.entries(porRol),
      headStyles: { fillColor: logoAzul },
      theme: 'striped'
    });

    doc.setFont("helvetica", "bold");
    doc.text("Conclusiones de Triangulación:", 20, (doc as any).lastAutoTable.finalY + 20);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    const conclusion = "Se observa una participación activa de los estamentos. Este documento sirve como base para el Plan de Formación Integral (PFI) y la armonización de documentos institucionales.";
    doc.text(doc.splitTextToSize(conclusion, 170), 20, (doc as any).lastAutoTable.finalY + 30);

    const pdfBase64 = doc.output('datauristring').split(',')[1];

    // 3. ENVÍO DE CORREO
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: 'leorozco1970@gmail.com',
        pass: 'mdso vzyq xaju vavn'
      }
    });

    await transporter.sendMail({
      from: '"Diagnóstico Institucional" <leorozco1970@gmail.com>',
      to: destinoCorreo,
      subject: `📊 Reporte Institucional: ${nombreInstitucion}`,
      text: `Adjunto enviamos el informe de triangulación de datos para la institución ${nombreInstitucion}.`,
      attachments: [{
        filename: `Informe_${nombreInstitucion.replace(/\s+/g, '_')}.pdf`,
        content: pdfBase64,
        encoding: 'base64'
      }]
    });

    return res.status(200).json({ success: true });
  } catch (error: any) {
    console.error("ERROR REPORTE:", error);
    return res.status(500).json({ error: error.message });
  }
}