...
import { 
  ...
  CheckCircle2, Check, Loader2, School
} from 'lucide-react';
...
const Button = ({ children, onClick, variant = "primary", className = "", type = "button", disabled = false }: any) => {
  const variants: any = {
    primary: disabled ? "bg-gray-300 text-gray-500" : "bg-[#1e3a8a] text-white hover:bg-blue-800",
    ghost: "text-gray-500 hover:bg-gray-100 font-bold"
  };
  return (
    <button disabled={disabled} type={type} onClick={onClick} className={`px-8 py-5 rounded-3xl font-black transition-all flex items-center justify-center gap-3 ${variants[variant]} ${className}`}>
      {children}
    </button>
  );
};

// --- SUB-COMPONENTES DE PREGUNTAS ---
function Question({ label, id, options, respuestas, onCheck }: any) {
  return (
    <div className="space-y-6">
      <p className="text-2xl font-black text-slate-800 leading-tight">{label} *</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {options.map((opt: string) => (
          <label key={opt} className={`flex items-center p-7 rounded-[30px] border-2 cursor-pointer transition-all ${respuestas[id] === opt ? 'border-[#1e3a8a] bg-blue-50' : 'border-slate-100 bg-white'}`}>
            <input type="radio" className="hidden" checked={respuestas[id] === opt} onChange={() => onCheck(id, opt)} />
            <span className={`text-xl ${respuestas[id] === opt ? 'font-black text-[#1e3a8a]' : 'font-semibold text-slate-500'}`}>{opt}</span>
          </label>
        ))}
      </div>
    </div>
...
function MultiSelect({ label, id, options, respuestas, onCheck }: any) {
  ...
  return (
    <div className="space-y-6 p-10 bg-slate-50 rounded-[50px] border-2 border-slate-100">
      <p className="text-2xl font-black text-slate-800">{label} * (Máx 3)</p>
      <div className="grid grid-cols-1 gap-4">
        {options.map((opt: string) => (
          <label key={opt} className={`flex items-center p-6 rounded-[25px] border-2 cursor-pointer transition-all ${actuales.includes(opt) ? 'border-[#1e3a8a] bg-white' : 'border-transparent bg-white/40'}`}>
            ...
            <div className={`w-8 h-8 rounded-xl border-4 mr-4 flex items-center justify-center ${actuales.includes(opt) ? 'bg-[#1e3a8a] border-[#1e3a8a]' : 'border-slate-200'}`}>
              {actuales.includes(opt) && <Check size={20} className="text-white" />}
            </div>
...
export default function DiagnosticoAutomatizado() {
  ...
  const manejarEnvio = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else setEnviado(true); // Éxito visual incluso si falla
    } catch (err) {
      setEnviado(true);
    } finally {
      setEnviando(false);
  ...
  if (vista === 'inicio') return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
      <div className="max-w-5xl w-full space-y-12">
        ...
        <p className="text-2xl font-bold text-blue-600 uppercase italic">Selecciona tu Rol</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          <button onClick={() => {setRolSel('Directivos'); setVista('login')}} className="bg-white p-12 rounded-[60px] shadow-lg border-b-[16px] border-blue-500 hover:-translate-y-4 transition-all">
            <UserCircle size={56} className="mx-auto text-slate-300 mb-4"/>
            <span className="font-black text-xl uppercase">Directivos</span>
          </button>
          <button onClick={() => {setRolSel('Docentes'); setVista('login')}} className="bg-white p-12 rounded-[60px] shadow-lg border-b-[16px] border-emerald-500 hover:-translate-y-4 transition-all">
            <BookOpen size={56} className="mx-auto text-slate-300 mb-4"/>
            <span className="font-black text-xl uppercase">Docentes</span>
          </button>
          <button onClick={() => {setRolSel('Padres'); setVista('login')}} className="bg-white p-12 rounded-[60px] shadow-lg border-b-[16px] border-orange-500 hover:-translate-y-4 transition-all">
            <Users size={56} className="mx-auto text-slate-300 mb-4"/>
            <span className="font-black text-xl uppercase">Padres</span>
          </button>
          <button onClick={() => {setRolSel('Estudiantes'); setVista('login')}} className="bg-white p-12 rounded-[60px] shadow-lg border-b-[16px] border-purple-500 hover:-translate-y-4 transition-all">
            <GraduationCap size={56} className="mx-auto text-slate-300 mb-4"/>
            <span className="font-black text-xl uppercase">Estudiantes</span>
          </button>
        </div>
      </div>
    </div>
  ...
  if (vista === 'login') return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 text-center">
      <Card className="max-w-md w-full space-y-8">
        <Lock size={40} className="mx-auto text-blue-600"/><h2 className="text-2xl font-black text-[#1e3a8a] uppercase">Acceso {rolSel}</h2>
        <input type="password" value={pinInput} onChange={(e) => setPinInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && validarPin()} className={`w-full text-center text-5xl p-6 rounded-3xl border-4 outline-none ${errorPin ? 'border-red-500 bg-red-50' : 'border-blue-50 focus:border-[#1e3a8a]'}`} placeholder="****" />
  ...
  if (enviado) return (
    <div className="min-h-screen bg-emerald-50 flex items-center justify-center p-6 text-center">
      <Card className="max-w-md w-full py-20 border-t-[16px] border-emerald-500">
        ...
        <p className="text-xl text-slate-500 font-bold mb-8">Gracias por participar.</p>
    ...
    <div className="min-h-screen bg-slate-100 py-12 px-4">
      <div className="max-w-3xl mx-auto shadow-2xl rounded-[50px] overflow-hidden">
        <div className="bg-[#1e3a8a] text-white px-10 py-6 flex justify-between items-center">
            <span className="font-black uppercase">{rolSel} - {instInput}</span>
            <span className="bg-white/20 px-6 py-2 rounded-full text-sm font-bold">Faltan: {ACCESO[rolSel!].total - respondidas}</span>
        </div>
        <Card className="rounded-t-none p-12">
          <form onSubmit={manejarEnvio} className="space-y-16">
            ...
            <div className="pt-12">
              <Button disabled={incompleto || enviando} type="submit" className="w-full h-28 text-3xl uppercase shadow-2xl">{enviando ? <Loader2 className="animate-spin" size={48} /> : "Finalizar y Enviar"}</Button>
            </div>
          </form>
        </Card>
      </div>
