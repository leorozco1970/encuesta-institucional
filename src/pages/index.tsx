...
body: JSON.stringify({ institucion: instInput.toUpperCase(), correo: emailInput.toLowerCase(), rol: rolSel, respuestas: arrayRespuestas }),
});
if (res.ok) setEnviado(true);
} catch (err: any) { alert(err.message); } finally { setEnviando(false); window.scrollTo(0,0); }
};

const dispararReporte = async () => {
const inst = prompt("Nombre de la institución:");
const mail = prompt("Correo:");
if(!inst || !mail) return;
setGenerandoReporte(true);
try {
const res = await fetch('/api/reporte', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombreInstitucion: inst, destinoCorreo: mail }) });
if(res.ok) alert("¡Enviado!");
} catch (e) { alert("Error"); } finally { setGenerandoReporte(false); }
};
...
<h2 className="text-4xl font-black text-slate-800 uppercase italic">¡EXITOSO!</h2>
...
<div className="bg-[#1e3a8a] text-white px-10 py-6 flex justify-between items-center"><span className="font-black uppercase">{rolSel} - {instInput}</span></div>
<div className="p-12">
...
<div className="pt-12"><Button disabled={incompleto || enviando} type="submit" className="w-full h-28 text-3xl uppercase">{enviando ? "..." : "Enviar"}</Button></div>
</form>
</div>
</div>
</div>
...
return (
<button onClick={onClick} className={bg-white p-12 rounded-[60px] shadow-lg border-b-[16px] transition-all hover:-translate-y-4 flex flex-col items-center gap-6 ${color}}>
<div className="text-slate-300">{icon}</div><span className="font-black text-slate-700 text-xl uppercase italic">{label}</span>
</button>
);
}